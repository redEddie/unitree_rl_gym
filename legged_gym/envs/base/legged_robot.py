...
class LeggedRobot(BaseTask):
    ...
    def _update_terrain_curriculum(self, env_ids):
        ...
        # 로봇이 자신의 지형 영역을 벗어난 경우 (지형의 절반 이상을 이동한 경우)
        move_up = distance > self.terrain.env_length / 2
        # 로봇이 아직 지형 안에 있는 경우 (지형의 절반 이하를 이동한 경우)
    ...
    def _reward_dof_vel_limits(self):
        ...
        return torch.sum((torch.abs(self.dof_vel) - self.dof_vel_limits*self.cfg.rewards.soft_dof_vel_limit).clip(min=0.0, max=1.), dim=1)
